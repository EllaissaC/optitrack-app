...
- **Inventory**: Full CRUD table with search and status filtering; Profit column (retail − wholesale); barcode scanning (USB scanner-compatible); manufacturer→brand dependent dropdowns (DB-backed); wholesale cost + multiplier → retail price auto-calculation; **Code field** (manufacturer color code/SKU variant, e.g. "8356") shown alongside Color in form, card, and table; frame variants share Brand+Model parent — duplicate detection uses Brand+Model+Code+Size+Bridge+Temple; invoice import extracts `code` from spreadsheet color-code columns and AI extraction
